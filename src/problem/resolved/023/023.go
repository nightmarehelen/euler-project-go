 ...
 */
 ...
 func main(){
 		...
 		for key,_ := range dict{
 			if i > key{
				...
				if ok{
					...
					fmt.Printf("isAbundant(%d)=%v\n", key, isAbundant(key))
					fmt.Printf("isAbundant(%d)=%v\n", i-key, isAbundant(i-key))
					flag = true
					break
				}
			}
 	...
 	fmt.Printf("%d is the sum of all the positive integers which cannot be written as the sum of two abundant numbers\n", sum)
 }
